...
### Lightbox palette preset

The palette exposes `ZLightbox` plus a lightbox trigger element. `ZLightbox` is a non-container Zaux component storing `id`, `items` (an array of image objects) and `options` (bigger-picture options). `integrations/zaux/controls/lightbox-controls.js` holds its preset; `app/services/catalog.js` gives each inserted gallery a unique `lightbox-<id>`. The Inspector uses `BuilderLightbox.vue` and `BuilderLightboxItems.vue` to edit the gallery ID, options and the image list (add, edit, duplicate, remove, reorder).

The trigger is opt-in rather than a dedicated preset. The HTML `button` element stays a plain button. Any element (button, a, div, ZButton, Zimg, …) can become a trigger by adding the `data-zlightbox-trigger` marker through the Inspector's **Add properties** list; `integrations/zaux/descriptors/property-descriptors.js` injects that descriptor for every component. Once the marker is present, `BuilderLightboxTrigger.vue` takes over the Inspector with the dual mode (open the gallery or a specific image index via `data-index`) and a target-gallery select populated from the trees in the template. A "remove trigger" action deletes the marker and its companions.
