<template>
  <aside class="zb-sidebar flex min-h-0 w-[254px] shrink-0 flex-col border-r-slim border-zaux-light-grey bg-zaux-white max-[1200px]:w-[230px] max-[900px]:h-[80dvh] max-[900px]:!w-[210px]" :style="{ width: `${width}px` }">
    <div class="zb-project-switch px-2 pb-2 pt-2">
      <div class="zb-section-heading mb-1 flex items-center justify-between"><span class="zb-eyebrow block text-[10px] font-semibold uppercase tracking-[1.4px] text-zaux-dark-grey">{{ translate('zx_builder_templates') }}</span><BuilderButton icon="add" iconOnly :label="translate('zx_builder_new_template')" @click="modal = { type: 'new-template' }" /></div>
      <BuilderInput type="select" :modelValue="activeTemplate.id" :label="translate('zx_builder_templates')" :options="document.templates.map(template => ({ value: template.id, label: template.name }))" @update:modelValue="selectTemplate" />
      <div class="zb-template-actions mt-1.5 flex justify-between gap-1 text-[10px] text-zaux-dark-grey [&>button:hover]:text-zaux-accent">
        <button @click="modal = { type: 'rename', kind: 'template', id: activeTemplate.id, name: activeTemplate.name }">{{ translate('zx_builder_rename') }}</button>
        <button @click="duplicate('template', activeTemplate.id)">{{ translate('zx_builder_duplicate') }}</button>
        <button @click="modal = { type: 'delete', kind: 'template', id: activeTemplate.id }">{{ translate('zx_builder_delete') }}</button>
      </div>
    </div>
