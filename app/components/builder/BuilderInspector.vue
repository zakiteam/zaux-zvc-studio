        ...
        <template v-if="activeDefinition">
          ...
          <template v-if="inspectorTab === 'properties'">
            <template v-if="selectedNode && !isSource">
              ...
              <details><summary>{{ translate('zx_builder_advanced') }}</summary><textarea v-model="propsDraft" rows="14" class="zb-code-input !bg-zaux-light !font-mono !text-[11px] !leading-[1.7] whitespace-pre [tab-size:2]" :aria-label="translate('zx_builder_advanced')" spellcheck="false" /><BuilderButton :label="translate('zx_builder_apply')" @click="applyProps" /></details>
            </template>
          ...
          <template v-else>
            ...
            <template v-if="codeMode === 'json'"><textarea v-model="definitionDraft" :readonly="isSource" rows="26" class="zb-code-input !bg-zaux-light !font-mono !text-[11px] !leading-[1.7] whitespace-pre [tab-size:2]" aria-label="ZVC JSON" spellcheck="false" /><BuilderButton v-if="!isSource" :label="translate('zx_builder_apply')" @click="applyDefinition" /></template>
            <template v-else-if="codeMode === 'js'"><textarea :value="source" readonly rows="26" class="zb-code-input !bg-zaux-light !font-mono !text-[11px] !leading-[1.7] whitespace-pre [tab-size:2]" aria-label="ZVC JavaScript" spellcheck="false" /><BuilderButton :label="translate('zx_builder_download_js')" @click="modal = { type: 'export', scope: 'component' }" /></template>
            <template v-else><p class="zb-help !mb-2 !mt-1.5 text-[11px] leading-[1.65] text-zaux-dark-grey">{{ translate('zx_builder_css_hint') }}</p><textarea :disabled="isSourceBase" :value="activeDefinition.css" rows="22" class="zb-code-input !bg-zaux-light !font-mono !text-[11px] !leading-[1.7] whitespace-pre [tab-size:2]" aria-label="CSS" spellcheck="false" @change="updateDefinition(def => { def.css = $event.target.value; })" /></template>
          </template>
        </template>
...
export default defineComponent({
  components: { BuilderButton, BuilderProperty, BuilderValue, BuilderFields, BuilderSourceInfo, BuilderInput },
  ...
  setup() {
    ...
    return { ...builder, visibleFields, fieldInputType, catalog, descriptors, visibleProperties, extraProperties, propsDraft, definitionDraft, source, localError, codeMode, setProperty, addProperty, changeType, applyProps, applyDefinition, fieldValue, setFieldValue };
  }
});
</script>
