		...
		>
			<div class="p-2 zb-inspector-content">
				<template v-if="activeDefinition">
					...
					<template v-else-if="inspectorTab === 'fields'"
						><p
							v-if="isSource"
							class="zb-help !mb-2 !mt-1.5 text-[11px] leading-[1.65] text-zaux-dark-grey"
						>
						...
						<BuilderFields v-else :key="activeDefinition.id"
					/></template>
					<template v-else>
						<div
							class="zb-field mb-2.5 [&>label]:mb-1 [&>label]:block [&>label]:text-[11px] [&>label]:font-medium [&>label]:text-zaux-dark [&_label_small]:mt-0.5 [&_label_small]:block [&_label_small]:font-mono [&_label_small]:text-[9px] [&_label_small]:text-zaux-dark-grey"
	...
	setup() {
		...
		function applyProps() {
			try {
				...
				localError.value = "zx_builder_invalid_json";
			}
		}
		function applyDefinition() {
			try {
				...
				localError.value = exception.message.startsWith("zx_")
					? exception.message
