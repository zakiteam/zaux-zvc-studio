  ...
  <div class="flex flex-col px-1 shrink-0 bg-zaux-white border-b-slim border-zaux-light-grey">
    <div class="flex min-w-0 items-center justify-center gap-1.5 pb-1">
      ...
      <button type="button" class="min-w-0 truncate rounded-xxs px-1 py-0.5 text-left text-[12px] font-semibold text-zaux-accent hover:bg-zaux-light focus-visible:outline focus-visible:outline-1 focus-visible:outline-zaux-accent"
        ...
        :aria-pressed="templatesOpen" :disabled="remoteProjectBusy" @click="templatesOpen = !templatesOpen">
        {{ activeTemplate.name }}
      </button>
    </div>
    <div v-if="!templatesOpen" class="flex shrink-0 flex-wrap justify-between items-top gap-x-3 gap-y-1 max-[1200px]:px-1 pb-1">
      <div class="flex flex-col gap-1">
        ...
        <div class="flex items-center gap-2 pt-0.5 pb-1">
          <BuilderButton size="xs" variant="alt1" swatch="#ffffff" :label="translate('zx_builder_canvas_light')"
            :class="{ '!bg-zaux-white': !canvasDark }" :aria-pressed="!canvasDark" @click="canvasDark = false" />
          <BuilderButton size="xs" variant="alt1" swatch="#18181b" :label="translate('zx_builder_canvas_dark')"
            :class="{ '!bg-zaux-white': canvasDark }" :aria-pressed="canvasDark" @click="canvasDark = true" />
        </div>
      </div>
