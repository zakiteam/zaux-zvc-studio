<template>
  <div class="zb-app flex h-screen overflow-hidden bg-zaux-light font-builder text-[13px] text-zaux-dark max-[700px]:flex-col">
    <aside class="flex h-full w-[240px] shrink-0 flex-col border-r-slim border-zaux-light-grey bg-zaux-white p-2 max-[700px]:w-full max-[700px]:border-b-slim">
      <NuxtLink to="/" class="mb-6 flex items-center gap-1.5 pb-1" aria-label="Zaux Studio">
      ...
      <nav :aria-label="translate('zx_builder_hub_navigation')" class="flex flex-col gap-1">
        ...
        <button type="button" class="rounded-xs px-2 py-1.5 text-left hover:bg-zaux-light" @click="fontsOpen = true">
      ...
      <div class="mt-auto border-t-slim border-zaux-light-grey pt-2 max-[700px]:mt-3">
        ...
        <BuilderDropdown 
          contentClass="py-2 flex flex-col gap-2" :label="translate('zx_builder_account')" 
              :extraTriggerProps="{
							iconName: 'user',
							hasIcon: true,
							hasActionIcon: false,
						}"
           :items="accountMenuItems" @select="accountAction" />
      </div>
    </aside>
    <BuilderFontLibrary v-if="fontsOpen" manage @close="fontsOpen = false" />
    <main class="min-w-0 flex-1 p-6 max-[700px]:p-3 overflow-y-scroll"><slot /></main>
  </div>
</template>
<script>
...
export default defineComponent({
  ...
  setup() {
    ...
    return { fontsOpen, studioLogo, user: auth.user, translate, accountMenuItems, accountAction };
  }
});
</script>
